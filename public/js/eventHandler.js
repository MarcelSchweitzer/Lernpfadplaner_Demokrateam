...
function searchBox() {
    ...
    const table = document.getElementById('infoTab');
...
function deleteOpenInteraction() {
    if (session.interactionOpened()) {
        ...
        if (session.interactionsExist && session.getCurrentScenario().interactions.length > 0) { session.openInteraction(session.getCurrentScenario().interactions.length - 1); }
...
function deleteIntType(category, interactionType) {
    const createdTypes = session.getCurrentLearningPath().lpSettings.createdTypes;
...
function resetSettings() {
    ...
    for (const categoryNames of Object.keys(session.getCurrentLearningPath().lpSettings.activeDefaultTypes)) {
...
document.addEventListener('click', (event) => {
    ...
    // download as json
        ...
        if (session.learningPathOpened()) {
            ...
            lpids = [];
            for (lp of session.getlearningPaths()) { lpids.push(lp.id); }
    ...
    // print a singe scenario
        ...
        const scenarioIndex = id.replaceAll('printScenario', '');
    ...
    // delete interactivity button in activity-Settings
        ...
        if (session.getCurrentLearningPath().lpSettings.ignoreWarnings) { deleteOpenInteraction(); } else { $('#deleteInteract').modal('show'); }
    }
    ...
    // reset the lp settings
        ...
        if (session.getCurrentLearningPath().lpSettings.ignoreWarnings) { resetSettings(); } else { $('#modalResetSettings').modal('show'); }
    }
    ...
    // confirm the copying of the lp settings of another lp
        ...
        const stealID = $('#stealSettingsDrop').children(':selected').attr('id').replaceAll('steal', '');
        session.setProp('lpSettings', session.getlearningPathById(stealID).lpSettings);
        if (document.getElementById('includeTaxo').checked) { session.setProp('taxonomyLevel', session.getlearningPathById(stealID).taxonomyLevel); }
        if (document.getElementById('includeEval').checked) { session.setProp('evaluationModeID', session.getlearningPathById(stealID).evaluationModeID); }
    ...
    // add Category Tab
        ...
        const allCatNames = Object.keys(session.getCurrentLearningPath().lpSettings.createdTypes).concat(Object.keys(session.getCurrentLearningPath().lpSettings.activeDefaultTypes)).concat('');
        const newCatName = uniqueName(document.getElementById('catNameGiven').value, allCatNames);

        document.getElementById('catNameGiven').value = '';
        const categoryID = newCatName.replaceAll(' ', '_');
        ...
        if (session.getCurrentLearningPath().lpSettings.createdTypes[category]) {
            let interactionTypes = session.getCurrentLearningPath().lpSettings.createdTypes[category];
            for (const interactionTypeName of Object.keys(interactionTypes)) {
        ...
        const category = id.replaceAll('catUnCheck ', '');
    ...
    // create new interactionType
        ...
        if (classes[2] === 'intDelBtn') { const category = classes[3]; } else { const category = classes[4]; }

        const interactionType = id.replaceAll('delCreInt', '');

        if (session.getCurrentLearningPath().lpSettings.ignoreWarnings) { deleteIntType(category, interactionType); } else {
        ...
        const interactionType = id;

        const catInCreate = session.getCurrentLearningPath().lpSettings.createdTypes[category];
        ...
        const category = id.replaceAll('catDel', '');

        if (session.getCurrentLearningPath().lpSettings.ignoreWarnings) { deleteCatType(category); } else {
        ...
        const category = id.replaceAll('catRename', '');
        ...
        const newCategory = document.getElementById('newCatNameGiven').value;
        const newCatID = newCategory.replaceAll(' ', '_');
...
document.addEventListener('input', (event) => {
    ...
    // handle changes of checkbox elements for default categorys
        ...
        const category = input.getAttribute('class').replaceAll('defaultInputCB', '');
        const interactionType = id.replaceAll('CB', '');

        activeDefaultTypes = session.getCurrentLearningPath().lpSettings.activeDefaultTypes;

        if (checked) {
            if (!activeDefaultTypes[category].includes(interactionType)) { activeDefaultTypes[category].push(interactionType); }
        } else {
            for (let i = 0; i < activeDefaultTypes[category].length; i++) {
                if (activeDefaultTypes[category][i] === interactionType) { activeDefaultTypes[category].splice(i, 1); }
            }
        }
    ...
    // handle changes of checkbox elements for user-created categorys
        ...
        const checked = input.checked;
        const category = input.getAttribute('class').replaceAll('createdInputCB', '');
        const interactionType = id.replaceAll('CB', '');

        createdIntInCat = session.getCurrentLearningPath().lpSettings.createdTypes[category];

        if (checked) { createdIntInCat[interactionType] = true; } else { createdIntInCat[interactionType] = false; }
    ...
    // change category name
        ...
        const changeCategory = id.replaceAll('changeCatName-', '');
        const newCatName = input.value;

        const newInter = session.getCurrentLearningPath().interactionTypes;
...
document.addEventListener('dragover', (event) => {
    if (draggedInteraction != null && draggedInteraction.classList.contains('interactionTypeBox') && session.scenarioOpened()) { event.preventDefault(); }
}, false);

// handle dropable elements beeing dropped
document.addEventListener('drop', (event) => {

    const coordinates = { x: (event.offsetX - canvasManager.getUserOffset().x) / canvasManager.getScale(), y: (event.offsetY - canvasManager.getUserOffset().y) / canvasManager.getScale() };
    ...
    if (draggedInteraction != null && draggedInteraction.classList.contains('interactionTypeBox') && session.scenarioOpened()) {
        ...
        if (droppedTo.classList.contains('p5Canvas')) {
            const category = draggedInteraction.getAttribute('id').split('$$')[0];
            const interactionType = draggedInteraction.getAttribute('id').split('$$')[1];
            category.trim();
