...
function createTreegraph(learningPath) {
  ...
  for (let i = 0; i < learningPath.scenarios.length; i++) {
    if ('interactions' in learningPath.scenarios[i]) {
      const interactList = [];
      for (let j = 0; j < learningPath.scenarios[i].interactions.length; j++) {
        const interactDict = {
  ...
  let root = { name: learningPath.title, parent: 'null', children: scenarioList };

  nodeList.push(root);

  const parent = document.getElementById('treegraph');
  parent.innerHTML = '';
